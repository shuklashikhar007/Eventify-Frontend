...
type Event struct {
	...
	EventStartTime time.Time      `gorm:"not null" json:"event_start_time" binding:"required,datetime"`
	EventEndTime   time.Time      `gorm:"not null" json:"event_end_time" binding:"required,datetime"`
	...
	CreatedBy      User           `gorm:"foreignKey:CreatedByID;references:ID;constraint:OnUpdate:CASCADE,OnDelete:SET NULL;" json:"created_by"`
}
...
type EventUpdater struct {
	...
	Event            Event     `gorm:"foreignKey:RefEventID;references:EventID;constraint:OnDelete:CASCADE"`
